...
<script>
    ...
    function show_like_modal(postId, loginNickname) {
        ...
        let loginNickname_pushed_like = 0;
        // 해당 게시물의 좋아요 정보 가져오기
        $.ajax({
            type:"get",
            ...
            success: function (data){
                ...
                for(let i=0;i<like.length;i++){
                    if(like[i] !== loginNickname){
                        ...
                        loginNickname_pushed_like =1;

                    }
                }
                   // 로그인된 닉네임이 좋아요 눌렀을 경우 맨아래로 표시
                if(loginNickname_pushed_like === 1){
                    let like_article_content = "<div class='like_article'>";
                    like_article_content += "<div class='like_profile'>";
                    like_article_content += "<a href='${contextPath}/main/profile/userProfile.do?userNickname=" + loginNickname + "'>";
                    like_article_content += "<img class='img-profile-32px' src='/main/post/profileImageDownload.do?userNickname="+loginNickname+"' alt='프로필이미지'>";
                    like_article_content += "</a>";
                    like_article_content += "<div class='like_nickName'>"+loginNickname+"</div></div></div>";
                    content += like_article_content;
                }
...
<main>
    <c:forEach var="post" items="${postMap.postList}">
        ...
        <div class="modal" id="modal${post.postId}">
            <div class="close_modal" onclick="close_modal('${post.postId}',0)">X</div>
