...
<c:set var="post" value="${postInfo.postDTO}" />
<body>
<div id="upload_from_box">
    <form name="uploadForm" id="uploadFrm" action="update.do" method="post">
            ...
            <tr>
                <td colspan="2">
                    <div class="image_upload_box">
                        ...
                        <span>게시물 수정</span>
                        <button id="upload-btn" type="button" onclick="upload_check()">수정<i class="fa-solid fa-arrow-up-from-bracket"></i></button>
                        <hr>
                    </div>
                </td>
            </tr>
            <tr>
                <td>
                    <div class="image_preview">
                        <div class="slider">
                            <c:forEach var="image" items="${postInfo.imageDTO}">
                                <c:if test="${image.postId == post.postId}">
                                    <div class="image_div">
                                        <img src="/main/post/imageDownload.do?imageFileName=${image.fileName}&postId=${post.postId}" alt="이미지"/>
                                    </div>
                                </c:if>
                            </c:forEach>
                        </div>
                ...
                <td>
                    ...
                    <div class="content_box">
                        <textarea id="content_input" name="content" rows="15" cols="40" maxlength="1500" spellcheck="false" onkeyup="count()" placeholder="내용을 입력하세요">${post.content}</textarea>
                    </div>
                    ...
                    <div class="hashtag_box">
                        <c:forEach var="hashTags" items="${postInfo.tagDTO}">
                            <c:if test="${hashTags.postId == post.postId}">
                                <c:forEach var="tags" items="${hashTags.hashTag}">
                                    <span class='hashtag'><span class='hashTag'>#${tags}</span><button class='del_btn' type='button'>X</button></span>
                                </c:forEach>
                            </c:if>
                        </c:forEach>
                    </div>
